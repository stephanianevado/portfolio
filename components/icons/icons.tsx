...
const resolveSize = (size: Size | undefined): number | string => {
  if (typeof size === 'number') return size * 4
  if (typeof size === 'string') return size
  return 24
}

type SvgBaseProps = IconProps & SVGProps<SVGSVGElement>
...
const IconSvg = ({
  ...
  return (
    <svg
      ...
      height={dimension}
      color={color}
      style={hoverColor ? { transition: 'fill 0.5s' } : undefined}
      {...rest}>
      {children}
    </svg>
  )
...
export const Linkedin = ({ color = primaryBlack, ...props }: IconProps) => {
  return (
    <IconSvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" {...props}>
      <path
...
export const Line = ({ color = 'var(--border)' }: LineProps) => {
  ...
  return (
    <svg
      ...
      fill="none"
      xmlns="http://www.w3.org/2000/svg">
      <path opacity="0.4" d={`M0 2H${width}`} stroke={color} strokeWidth="4" />
    </svg>
  )
}
...
export const Dash = () => {
  return (
    <svg
      ...
      fill="none"
      xmlns="http://www.w3.org/2000/svg">
      <path d="M0 1H47" stroke="var(--muted-foreground)" strokeWidth="2" />
    </svg>
  )
}
